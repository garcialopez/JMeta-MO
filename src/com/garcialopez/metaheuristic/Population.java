...
public class Population {
    ...
    public static double[][] startPopulation(CNOP cnop, int sizeRow, boolean duplicateVariable) {
        ...
        if (numberObjective > 1) {
            sizeColumns = numberObjective + 1; //el 1 es el svr
        }
    ...
    public static double[][] sortPopulationMO(double[][] individual, CNOP cnop) {

        int matrixSize = individual.length;
        int svr = individual[0].length-1;
        int fx1 = svr - 2;
        int fx2 = svr - 1;
        ...
        for (int i = 0; i < matrixSize; i++) {
            ...
            for (int j = 0; j < (matrixSize - 1); j++) {
                comparison1 = CNOP.compare(individual[j][svr], 0, "==");
                comparison2 = CNOP.compare(individual[j + 1][svr], 0, "==");
                if (comparison1 && comparison2) {

                    comparison1 = CNOP.compare(individual[j + 1][fx1], individual[j][fx1], cnop.getObjetives()[0][1]);
                    comparison2 = CNOP.compare(individual[j + 1][fx2], individual[j][fx2], cnop.getObjetives()[1][1]);
                    if (comparison1 && comparison2) {
                        double[] temp = Arrays.copyOf(individual[j], individual[j].length);
                        individual[j] = Arrays.copyOf(individual[j + 1], individual[j + 1].length);
                        individual[j + 1] = Arrays.copyOf(temp, temp.length);
                ...
                if ((individual[j][svr] > 0) && (individual[j + 1][svr] > 0)) {
                    if (individual[j][svr] > individual[j + 1][svr]) {
                        double[] temp = Arrays.copyOf(individual[j], individual[j].length);
                        individual[j] = Arrays.copyOf(individual[j + 1], individual[j + 1].length);
                        individual[j + 1] = Arrays.copyOf(temp, temp.length);
                    }
