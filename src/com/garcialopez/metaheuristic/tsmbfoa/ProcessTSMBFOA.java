...
public class ProcessTSMBFOA implements Foraging {
    ...
    public void chemotaxis(CNOP cnop, TSMBFOA bacteria) {
        ...
        int fx1 = sizeValues - 3;
        int fx2 = sizeValues - 2;
        ...
        double[][] newBacterium = new double[1][bacteria.getIndividuals()[0].length];
        ...
        for (int b = 0; b < bacteria.getSb(); b++) { //for b
            ...
            for (int c = 0; c < bacteria.getNc(); c++) {   //start chemotaxis - for c                
                if (flag) {
                    System.arraycopy(this.generateAngles(numberVariables), 0, angles, 0, numberVariables);
                }

                //----------------------------------------------------------------------------
                int[] v = new int[3];                     //index for vast                
                double[] bact1 = new double[sizeValues];  //new childr
                double[] bact2 = new double[sizeValues];  //new childr  
                double[] bact3 = new double[sizeValues];  //new childr

                this.offspring(sizeIndividuals, b, v); //size, no include, container

                //preguntar si en bact1 es b o v1
                System.arraycopy(bacteria.getIndividuals()[v[0]], 0, bact1, 0, bact1.length);
                System.arraycopy(bacteria.getIndividuals()[v[1]], 0, bact2, 0, bact2.length);
                System.arraycopy(bacteria.getIndividuals()[v[2]], 0, bact3, 0, bact3.length);
                //----------------------------------------------------------------------------
                ...
                cnop.evaluateObjectiveFunction(newBacterium);
...
/*De Aqui if (newBacterium[0][indexSVR] == 0 && bacteria.getIndividuals()[b][indexSVR] == 0) {
                    if (newBacterium[0][indexFO] < bacteria.getIndividuals()[b][indexFO]) {
                        flag = false;
                        System.arraycopy(newBacterium[0], 0, bacteria.getIndividuals()[b], 0, sizeValues);
                    } else {
                        flag = true;
                    }
                }
                ...
                double mSvr = newBacterium[0][indexSVR];
                double nSvr = bacteria.getIndividuals()[b][indexSVR];

                boolean comparison1 = CNOP.compare(mSvr,0,"==");
                boolean comparison2 = CNOP.compare(nSvr,0,"==");

                if (comparison1 && comparison2) {
                    comparison1 = CNOP.compare(newBacterium[0][fx1], bacteria.getIndividuals()[b][fx1], cnop.getObjetives()[0][1]);
                    comparison2 = CNOP.compare(newBacterium[0][fx2], bacteria.getIndividuals()[b][fx2], cnop.getObjetives()[1][1]);
...
/*Hasta Aqui*/  }
            ...
            if (numberObjective > 1) {
                Population.sortPopulationMO(bacteria.getIndividuals(), cnop);
            } else {
                Population.sortPopulation(bacteria.getIndividuals());
            }
        }//for b
    ...
    @Override
    public void reproduction(TSMBFOA bacteria, int no, CNOP cnop) {
        if (bacteria.getGmax() % bacteria.getRepcycle() == 0) {
            ...
            for (int y = (row - bacteria.getBacteriaReproduce()); y < row; y++) {
                ...
                if (no > 1) {
                    System.arraycopy(bacteria.getIndividuals()[ind], 0, bacteria.getIndividualsMat()[y], 0, column);
                } else {
                }
                System.arraycopy(bacteria.getIndividuals()[ind], 0, bacteria.getIndividuals()[y], 0, column);

            }
            if (no > 0) {
                Population.sortPopulation(bacteria.getIndividualsMat());
            } else {
                Population.sortPopulationMO(bacteria.getIndividuals(), cnop);
            }
        }
    ...
    public void eliminationDispersal(CNOP cnop, TSMBFOA bacteria) {
        ...
        double[] angles = new double[numberVar];
        System.arraycopy(this.generateAngles(numberVar), 0, angles, 0, numberVar);

        double[][] bacter = new double[1][bacteria.getIndividuals()[0].length];
        ...
        cnop.evaluateObjectiveFunction(bacter);
        System.arraycopy(bacter[0], 0, bacteria.getIndividuals()[sizeInd - 1], 0, bacter[0].length);
        Population.sortPopulation(bacteria.getIndividuals());
    }
     ...
     */
    private int[] offspring(int limit, int different, int[] v) {
        // Creamos una lista para almacenar los índices en orden
        ...
        // Mezclamos la lista de índices de manera aleatoria
        Collections.shuffle(indices, new Random());

        // Tomamos los primeros tres elementos de la lista mezclada        
