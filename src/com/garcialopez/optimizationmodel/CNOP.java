...
//    private String type = CNOP.MINIMIZATION;
    ...
    /**
     * 
     * @param function is the objective function
     * @param type is CNOP.MAXIMIZATION or CNOP.MINIMIZATION
     */
    public void addObjective(String function, String type){
        
        
        functions.add(new String[]{Transform.extendFunction(function),type});
    }//close
    
    public String[][] getObjetives(){
        String[][] objetives = new String[this.functions.size()][2];
        
        for (int i = 0; i < this.functions.size(); i++) {
            objetives[i][0] = this.functions.get(i)[0];
            objetives[i][1] = this.functions.get(i)[1];
...
//        this.numberVariableOF = new Expression(this.function).getMissingUserDefinedArguments().length;
    ...
    public void parserCNOP() {
        ...
        this.parseCNOP.setExpressionOF(this.functions, this.orderVariables);
    ...
    public double[][] evaluateObjectiveFunction(double[][] values) {
        int numberObjetive = this.functions.size();
...
//        int indexFO = sizeValues - 2;
        ...
        for (double[] value : values) {
            ...
            int j = 0;
            for (int i = numberObjetive; i > 0; i--) {
                value[indexSVR - i] = this.parseCNOP.evaluateOF(j, valuesAux);
                j++;
            }
    ...
    public static boolean compare(double m, double n, String comparator){
        switch (comparator) {
