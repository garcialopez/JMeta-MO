...
public class ParseCNOP {
    ...
    private ArrayList<Expression> objetives;
     ...
     */
    public void setExpressionOF(List<String[]> expressionOF, List<String> variables) {

        this.objetives = new ArrayList();

        this.variables = variables;

        for (int i = 0; i < expressionOF.size(); i++) {
            this.objetives.add(this.loadFunction(expressionOF.get(i)[0]));
        }
    }
...
//    }
    ...
    public double evaluateOF(int index, double[] values) {
        this.objetives.set(index,
                this.setArguments(this.objetives.get(index),
                        values)
        );

        return this.objetives.get(index).calculate();
    }
